...
1. **关卡内容** — 管理章节与关卡、编辑旋转/指引公式、预览 3D 起终态
2. **AI 能力标签** — 维护内部能力维度（阶段、定义、掌握标准；不含公式）
3. **AI 推荐配置** — 一关一个主能力标签，设置教学模式与难度，发布检查后导出 App v1

可选接入通义千问（DashScope）生成公式候选；首次启动提供新手引导（也可从「帮助 → 新手引导」重看）。

更完整的**现状、进度与改代码索引**见 [`docs/`](./docs/README.md)（建议先读 [`docs/PROJECT_STATUS.md`](./docs/PROJECT_STATUS.md)）。
